...
{
    public partial class FRM_CONSUL_REAL_MENSUAL : Form
    {
        DataView oDv = new DataView();
        Metodos met = new Metodos();
        N_MAESTROS oSedN = new N_MAESTROS();
        N_REAL_INSUMOS oN = new N_REAL_INSUMOS();
        private static FRM_CONSUL_REAL_MENSUAL _Instancia;
        public static FRM_CONSUL_REAL_MENSUAL GetInstancia()
        {
            if (_Instancia == null)
            {
                _Instancia = new FRM_CONSUL_REAL_MENSUAL();
            }
            return _Instancia;
        }
        public FRM_CONSUL_REAL_MENSUAL()
        {
            InitializeComponent();
            cbo_Sede();
            cbo_Cultivo();
        ...
        {
            oDv = new DataView(oN.lst_REAL(3, sede, fundo, cultivo));
