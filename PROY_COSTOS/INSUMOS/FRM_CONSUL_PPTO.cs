...
{
    public partial class FRM_CONSUL_PPTO : Form
    {
        DataView oDv = new DataView();
        Metodos met = new Metodos();
        N_MAESTROS oSedN = new N_MAESTROS();
        N_PPTO_INSUMOS oN = new N_PPTO_INSUMOS(); 
        private static FRM_CONSUL_PPTO _Instancia;
        public static FRM_CONSUL_PPTO GetInstancia()
        {
            if (_Instancia == null)
            {
                _Instancia = new FRM_CONSUL_PPTO();
            }
            return _Instancia;
        }
        public FRM_CONSUL_PPTO()
        {
            InitializeComponent();
            cbo_Sede();
            cbo_Cultivo();
        ...
        private void lst_lPPTO(int sede, int fundo, int cultivo, int version)
        {
            oDv = new DataView(oN.lst_PPTO(sede, fundo, cultivo, 1));
        ...
        {
            lst_lPPTO(Convert.ToInt32(this.cboSede.SelectedValue.ToString()), Convert.ToInt32(this.cboFundo.SelectedValue.ToString()), Convert.ToInt32(this.cboCultivo.SelectedValue.ToString()), Convert.ToInt32(this.cboVersion.SelectedValue.ToString()));
