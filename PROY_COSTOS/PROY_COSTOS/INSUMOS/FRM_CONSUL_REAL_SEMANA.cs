...
{
    public partial class FRM_CONSUL_REAL_SEMANA : Form
    {
        DataView oDv = new DataView();
        Metodos met = new Metodos();
        N_MAESTROS oSedN = new N_MAESTROS();
        N_REAL_INSUMOS oN = new N_REAL_INSUMOS();
        private static FRM_CONSUL_REAL_SEMANA _Instancia;
        public static FRM_CONSUL_REAL_SEMANA GetInstancia()
        {
            if (_Instancia == null)
            {
                _Instancia = new FRM_CONSUL_REAL_SEMANA();
            }
            return _Instancia;
        }
        public FRM_CONSUL_REAL_SEMANA()
        {
            InitializeComponent();
            cbo_Sede();
            cbo_Cultivo();
        ...
        {
            oDv = new DataView(oN.lst_REAL(2, sede, fundo, cultivo));
